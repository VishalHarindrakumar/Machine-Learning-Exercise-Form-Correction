...
# Load audio files
audio_loose_upper_arm = "audio/loose_upper_arm.mp3"
audio_peak_contraction = "audio/peak_contraction.mp3"
pygame.mixer.music.load(audio_loose_upper_arm)
    ...
    def analyze_pose(
        ...
        if self.stage == "up" and bicep_curl_angle < self.peak_contraction_angle:
            ...
            if (
                ...
                has_error = True
                self.detected_errors["PEAK_CONTRACTION"] += 1
                pygame.mixer.music.load(audio_peak_contraction)
                pygame.mixer.music.play()                
...
# Function to play video with audio using moviepy
def play_video_with_audio():
    video_path = os.path.join("videos", "bicep_curl.mp4")
...
def start_detection():
    ...
    while cap.isOpened():
        ...
        if cv2.waitKey(5) & 0xFF == ord('q'):
            break
        print(f"Left Arm: {bicep_curl_detection.left_arm_analysis.counter} reps, {bicep_curl_detection.left_arm_analysis.error_count} errors")
        print(f"Right Arm: {bicep_curl_detection.right_arm_analysis.counter} reps, {bicep_curl_detection.right_arm_analysis.error_count} errors")

        if(bicep_curl_detection.left_arm_analysis.error_count > 150 or bicep_curl_detection.right_arm_analysis.error_count > 190):
            print("THRESHOLD REACHED")
            cap.release()
            cv2.destroyWindow('Bicep Curl Real-Time Analysis')
            ...
            if user_choice.lower() == 'y':
                ...
                bicep_curl_detection.left_arm_analysis.error_count=0
                bicep_curl_detection.right_arm_analysis.error_count=0
