...
# Load audio files for squat errors
audio_foot_too_tight = "audio/feet_too_tight.mp3"
audio_foot_too_wide = "audio/feet_too_wide.mp3"
audio_knee_too_tight = "audio/knee_too_tight.mp3"
audio_knee_too_wide = "audio/knee_too_wide.mp3"
...
class SquatDetection:
    ML_MODEL_PATH = "./models/squat_model.pkl"
    ...
    def detect(self, mp_results, image, timestamp) -> None:
        ...
        try:
            ...
            # Play audio for foot placement errors
            if foot_placement_evaluation == 1:
                pygame.mixer.music.load(audio_foot_too_tight)
                pygame.mixer.music.play()
            elif foot_placement_evaluation == 2:
                pygame.mixer.music.load(audio_foot_too_wide)
                pygame.mixer.music.play()

            # Play audio for knee placement errors
            if knee_placement_evaluation == 1:
                pygame.mixer.music.load(audio_knee_too_tight)
                pygame.mixer.music.play()
            elif knee_placement_evaluation == 2:
                pygame.mixer.music.load(audio_knee_too_wide)
                pygame.mixer.music.play()
...
def play_video_with_audio():
    video_path = os.path.join("videos", "squat.mp4")
...
def start_detection():
    ...
    while cap.isOpened():
        ...
        if mp_results.pose_landmarks:
            ...
            SquatDetection.detect(mp_results, image, current_time)

        # Display the processed image
        cv2.imshow('Squat Real-Time Analysis', image)
        if cv2.waitKey(5) & 0xFF == ord('q'):
            break

        print(f"SQUAT ERROR: {SquatDetection.error_count}")
        if SquatDetection.error_count>250:
            print("THRESHOLD REACHED")
            cap.release()
            cv2.destroyWindow('Squat Real-Time Analysis')
